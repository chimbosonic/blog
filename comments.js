...
{
    return BBB_MASTODON_COMMENTS_BLACKLIST.has(rec.id) ||
...
function on_sensitive_button_click(ev) {
    ...
    while (content_outer && !content_outer.classList.contains('mast-comment-content-outer'))
        content_outer = content_outer.parentElement;

    if (!content_outer)
        throw "can't find mast-comment-content-outer";
...
function emoji_fix(rec) {
    ...
    function replace_emoji(match, shortcode) {
        ...
        if (url)
            return `<img class="mast-comment-emoji" src="${url}">`;

        return `:${shortcode}:`
    }

    const content = rec.content.replace(/:(\S+?):/g, replace_emoji);
    rec.content = content;
}
...
function content_fix(rec) {
    if (rec.content.substring(0,3) != "<p>")
        rec.content = "<p>" + rec.content;
}
...
function get_attachment_html(rec) {
    ...
    for (a of rec?.media_attachments) {
        switch (a.type) {
            case 'image': icon = "ðŸ–¼ï¸"; break;
            case 'gifv': icon = "ðŸŽ¦"; break;
            case 'video': icon = "ðŸŽ¦"; break;
            case 'audio': icon = "ðŸŽ¶"; break;
            default: icon = "ðŸ“Ž"; break;
        }

        const desc = a.description? a.description: `Attachment (${a.type})`;
        const line = `<div class="mast-comment-attachment">${icon}&nbsp;<a href="${a.url}">${desc}</a></div>`;
        html += line;
    }
